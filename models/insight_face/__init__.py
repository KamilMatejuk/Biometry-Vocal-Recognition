...
# from models.insight_face.src.models import FocalLoss, ResNetFace, IRBlock, InsightMarginProduct

from models import Model, Preprocessor
from loggers import Insight_face_logger as logger


class InsightFacePreprocessorTrain(Preprocessor):
    @staticmethod
...
class InsightFacePreprocessorTest(Preprocessor):
    @staticmethod
    def preprocess(image: Image) -> torch.Tensor:
        image = image.convert('L')
        trans = transforms.Compose([
...
class InsightFaceModel(Model):
    def __init__(self, device: str) -> None:
        ...
        # self.metric_fc = InsightMarginProduct(512, self.num_classes, s=30, m=0.5, easy_margin=False)
